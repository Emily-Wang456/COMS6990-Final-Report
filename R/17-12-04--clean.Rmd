...
cat("\n Original missing rate is", sum(is.na(mdat)) / (14448*348))
```
...
cat("Original number of countries is:", length(unique(sub_mdat_pre$Country.Code)))
```
...
#length(unique(sub.mdat.plot.ts$Country.Code))
#(is.na(sub.mdat.plot.ts))
```
...
cat("the missing rate after imputation is:", sum(is.na(after.imputation)) / 101088)
```
